...
- **웹 터미널**: 브라우저에서 직접 터미널 명령어 실행 (git, npm 등)
...
### 1. 블로그 루트 설정
GitBlogEditor를 블로그 디렉토리와 같은 레벨에 배치하거나, `server/app.js`에서 `BLOG_ROOT` 경로를 수정하세요.
...
### 블로그 루트 경로 설정
`server/app.js`에서 `BLOG_ROOT` 변수를 블로그 디렉토리 경로로 수정하세요:

```javascript
const BLOG_ROOT = path.join(__dirname, '../../your-blog-directory');
```
...
이 프로젝트는 GitHub Pages 블로그 관리의 불편함을 해결하기 위해 만들어졌습니다. 
웹 기반 편집기와 터미널 통합으로 더 효율적인 블로그 관리를 경험해보세요!
